...
// TODO: save for later. to make a fortiweb-autoscale-core node module distribution
function makeDistCore() {

}

// TODO: save for later. to make a fortiweb-autoscale-aws node module distribution
function makeDistAWS() {

}

// TODO: save for later. to make a fortiweb-autoscale-azure node module distribution
...
async function makeDistAWSLambda(options = {saveToDist: 'zip', keepTemp: false}) {
    ...
    let rTempDir = await makeTempDir(),
        ...
        rTempDirSrcAwsDep = path.resolve(rTempDirSrcLib, 'aws_depend'),
        packageInfo,
        zipFilePath,
        rDirSrcCore = path.resolve(REAL_PROJECT_ROOT, './core'),
        rDirSrcAws = path.resolve(REAL_PROJECT_ROOT, './aws'),
        rDirSrcAwsDep = path.resolve(REAL_PROJECT_ROOT, './aws_depend'),
    ...
    await copyAndDelete(rDirSrcAws, rTempDirSrcAws,
        ['node_modules', 'local*', 'test', '.nyc_output', '.vscode', 'package-lock.json']);
    //llf
    await copyAndDelete(rDirSrcAwsDep, rTempDirSrcAwsDep,
        ['node_modules', 'local*', 'test', '.nyc_output', '.vscode', 'package-lock.json']);
    ...
    await npmInstallAt(rTempDirSrcLambda,
        ['--save', rTempDirSrcAwsDep.concat('/uuid').replace(rTempDirSrcLambda, '.')]);
...
async function makeDistAwsCloudFormation(options = {excludeList: [], quickstart: false}) {
    ...
    let rTempDirSrcValidateLambda = path.resolve(rTempDir, 'aws_validate_lambda'),
        rDirSrcValidateLambda = path.resolve(REAL_PROJECT_ROOT, './aws_cloudformation/templates/validate_lambda.py'),
        validateLambdaZipFilePath;
    let rTempDirSrcFindAMILambda = path.resolve(rTempDir, 'aws_find_ami_lambda'),
    ...
    //copy and move validate lambda
    await makeDir(rTempDirSrcValidateLambda);
    ...
    await copy(rDirSrcFindAMILambda, rTempDirSrcFindAMILambda);
    FindAMILambdaZipFilePath = await zipSafe('find_ami.zip', rTempDirSrcFindAMILambda);
    await moveSafe(FindAMILambdaZipFilePath, rTempDirFunctionPackages);
    ...
    zipFileName = options.quickstart ? 'fortiweb-autoscale-aws-quickstart.zip' :
        'fortiweb-autoscale-aws-cloudformation.zip';
...
async function makeDistProject() {
    ...
    let realTmpDir = await makeTempDir(),
        ...
        realPjtDir = path.resolve(REAL_PROJECT_ROOT, './*'),
        packageInfo,
        zipFileName,
        realZipFilePath,
        saveAsFilePath;
    ...
    await copyAndDelete(realPjtDir, realTmpSrcDir, ['node_modules', 'dist', 'local',
...
async function makeDistAzureQuickStart() {
    ...
    zipFileName = 'fortiweb-autoscale-azure-quickstart.zip';
